...
export class WalletController {
  ...
  async getUserWallet(request: FastifyRequest, reply: FastifyReply) {
    try {
  ...
  async getWalletUsageHistory(request: FastifyRequest, reply: FastifyReply) {
    try {
      const userId = (request as any).user.id;
      const { page = 1, limit = 10 } = request.query as any;
      
      const result = await this.walletService.getWalletUsageHistory(userId, Number(page), Number(limit));
      ...
      return reply.code(500).send({
        success: false,
        message: 'Failed to fetch wallet usage history',
      });
    }
  }
}
